...
export default function SignUp() {
  ...
  const mapApiError = (e: any): string => {
    ...
    if (status) {
      if (status >= 500) {
        ...
        message = 'Não foi possível processar os dados informados. Verifique e tente novamente.';
      } else {
  ...
  const errors = useMemo(() => {
    ...
    if (!passwordConfirmation) e.password_confirmation = 'Confirmação é obrigatória.';
    ...
    if (role === 'teacher') {
      ...
      if (!schoolEmail.trim()) e.schoolEmail = 'E-mail da escola é obrigatório.';
  ...
  async function loadSchools() {
    try {
      ...
      if (e?.request && !e?.response) {
        ...
        try { Alert.alert('Erro ao carregar escolas', msg); } catch {}
      } else {
        const msg = mapApiError(e);
        setSchools([]);
        setSchoolsMessage('Nenhuma escola encontrada, tente novamente mais tarde.');
        try { Alert.alert('Erro ao carregar escolas', msg); } catch {}
      }
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = encodeURIComponent('Cadastro efetuado com sucesso. Efetue o login.');
      router.replace(`/auth/sign-in?msg=${message}`);
    } catch (e: any) {
      const msg = mapApiError(e);
      setSubmitError(msg);
      try { Alert.alert('Erro no cadastro', msg); } catch {}
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <ScrollView ref={scrollRef} contentContainerStyle={{ padding: 24, backgroundColor: 'white' }}>
      ...
      {submitError ? (
        <View style={{ backgroundColor: '#fee2e2', borderWidth: 1, borderColor: '#fca5a5', padding: 12, borderRadius: 8, marginBottom: 16 }}>
          <Text style={{ color: '#b91c1c' }}>{submitError}</Text>
        </View>
      ) : null}
      ...
      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16 }}>
        <TouchableOpacity
          ...
          style={{ padding: 10, borderRadius: 8, borderWidth: 1, borderColor: role === 'teacher' ? '#2563eb' : '#ddd', backgroundColor: role === 'teacher' ? '#dbeafe' : 'white' }}
        >
          <Text style={{ color: '#111827' }}>Professor</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => setRole('admin')}
          style={{ padding: 10, borderRadius: 8, borderWidth: 1, borderColor: role === 'admin' ? '#2563eb' : '#ddd', backgroundColor: role === 'admin' ? '#dbeafe' : 'white' }}
        >
          <Text style={{ color: '#111827' }}>Admin</Text>
        </TouchableOpacity>
      </View>

      {role === 'admin' && (
        <View style={{ marginBottom: 20 }}>
          <Text style={{ fontSize: 18, fontWeight: '600', marginBottom: 10 }}>Dados da Escola</Text>
          <Text style={{ marginBottom: 6 }}>Nome da Escola <Text style={{ color: '#dc2626' }}>*</Text></Text>
          <TextInput
            value={schoolName}
            onChangeText={setSchoolName}
            onBlur={() => setTouched((p) => ({ ...p, schoolName: true }))}
            placeholder="Nome da Escola"
            style={{ borderWidth: 1, borderColor: touched.schoolName && errors.schoolName ? '#dc2626' : '#ddd', borderRadius: 8, padding: 12, marginBottom: 6 }}
          />
          {touched.schoolName && errors.schoolName ? (
            <Text style={{ color: '#dc2626', marginBottom: 10 }}>{errors.schoolName}</Text>
          ) : <View style={{ height: 10 }} />}
          <Text style={{ marginBottom: 6 }}>E-mail da Escola <Text style={{ color: '#dc2626' }}>*</Text></Text>
          <TextInput
            value={schoolEmail}
            ...
            autoCapitalize="none"
            style={{ borderWidth: 1, borderColor: touched.schoolEmail && errors.schoolEmail ? '#dc2626' : '#ddd', borderRadius: 8, padding: 12, marginBottom: 6 }}
          />
          {touched.schoolEmail && errors.schoolEmail ? (
            <Text style={{ color: '#dc2626', marginBottom: 10 }}>{errors.schoolEmail}</Text>
          ) : <View style={{ height: 10 }} />}
          <Text style={{ marginBottom: 6 }}>Endereço</Text>
          <TextInput value={schoolAddress} onChangeText={setSchoolAddress} placeholder="Endereço (opcional)" style={{ borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 12, marginBottom: 10 }} />
          <Text style={{ marginBottom: 6 }}>Telefone</Text>
          <TextInput value={schoolPhone} onChangeText={setSchoolPhone} placeholder="Telefone (opcional)" keyboardType="phone-pad" style={{ borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 12 }} />
        </View>
      )}
      ...
      {role === 'teacher' && (
        <View style={{ marginBottom: 20 }}>
          ...
          {touched.selectedSchoolId && errors.selectedSchoolId ? (
            <Text style={{ color: '#dc2626', marginTop: 6 }}>{errors.selectedSchoolId}</Text>
          ) : null}
        </View>
      )}

      <View style={{ marginBottom: 20 }}>
        <Text style={{ fontSize: 18, fontWeight: '600', marginBottom: 10 }}>Seus Dados</Text>
        <Text style={{ marginBottom: 6 }}>Nome <Text style={{ color: '#dc2626' }}>*</Text></Text>
        <TextInput
          value={name}
          onChangeText={setName}
          onBlur={() => setTouched((p) => ({ ...p, name: true }))}
          placeholder="Seu nome"
          style={{ borderWidth: 1, borderColor: touched.name && errors.name ? '#dc2626' : '#ddd', borderRadius: 8, padding: 12, marginBottom: 6 }}
        />
        {touched.name && errors.name ? (
          <Text style={{ color: '#dc2626', marginBottom: 10 }}>{errors.name}</Text>
        ) : <View style={{ height: 10 }} />}
        <Text style={{ marginBottom: 6 }}>E-mail <Text style={{ color: '#dc2626' }}>*</Text></Text>
        <TextInput
          value={email}
          ...
          autoCapitalize="none"
          keyboardType="email-address"
          style={{ borderWidth: 1, borderColor: touched.email && errors.email ? '#dc2626' : '#ddd', borderRadius: 8, padding: 12, marginBottom: 6 }}
        />
        {touched.email && errors.email ? (
          <Text style={{ color: '#dc2626', marginBottom: 10 }}>{errors.email}</Text>
        ) : <View style={{ height: 10 }} />}
        <Text style={{ marginBottom: 6 }}>Senha <Text style={{ color: '#dc2626' }}>*</Text></Text>
        <TextInput
          value={password}
          ...
          placeholder="••••••••"
          secureTextEntry
          style={{ borderWidth: 1, borderColor: touched.password && errors.password ? '#dc2626' : '#ddd', borderRadius: 8, padding: 12, marginBottom: 6 }}
        />
        {touched.password && errors.password ? (
          <Text style={{ color: '#dc2626', marginBottom: 10 }}>{errors.password}</Text>
        ) : <View style={{ height: 10 }} />}
        <Text style={{ marginBottom: 6 }}>Confirme a Senha <Text style={{ color: '#dc2626' }}>*</Text></Text>
        <TextInput
          value={passwordConfirmation}
          onChangeText={setPasswordConfirmation}
          onBlur={() => setTouched((p) => ({ ...p, password_confirmation: true }))}
          placeholder="••••••••"
          secureTextEntry
          style={{ borderWidth: 1, borderColor: touched.password_confirmation && errors.password_confirmation ? '#dc2626' : '#ddd', borderRadius: 8, padding: 12, marginBottom: 6 }}
        />
      ...
      <TouchableOpacity
        ...
        style={{ backgroundColor: '#16a34a', padding: 14, borderRadius: 8, alignItems: 'center', opacity: submitting || loading ? 0.7 : 1 }}
      >
        <Text style={{ color: 'white', fontWeight: '600' }}>{submitting ? 'Enviando...' : 'Cadastrar'}</Text>
      </TouchableOpacity>

      <TouchableOpacity onPress={() => router.back()} style={{ alignItems: 'center', padding: 12 }}>
        <Text style={{ color: '#2563eb' }}>Já tem conta? Entrar</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
